package com.dtstack.trino.uitl;
...
public class IPAddressUtils {
    ...
    private static File ipTmp;
    /**
     * 纯真IP数据库保存的文件夹
     */
    private String INSTALL_DIR=System.getProperty("java.class.path");;//获取文件路径;
...
//                ipFile = new RandomAccessFile(IP_FILE, "r");
                ...
                BufferedInputStream bris = new BufferedInputStream(is);
                ipTmp = File.createTempFile("iptmp", ".dat");
                BufferedOutputStream bros = new BufferedOutputStream(new FileOutputStream(ipTmp));
                int cnt = 0;
                byte[] readBuff = new byte[1024];
                try {
                    while((cnt=bris.read(readBuff))!=-1) {
                        bros.write(readBuff);
                    }
                } catch (IOException e) {
                ...
                ipFile = new RandomAccessFile(ipTmp, "r");
...
//                    }
//                }
                log.error("IP地址信息文件没有找到，IP显示功能将无法使用:{}" + e.getMessage(), e);
            }
            // 如果打开文件成功，读取文件头信息
            if(ipFile != null) {
                try {
    ...
    public static void main(String[] args){
        ...
        ip.init();
//        String address = "112.225.35.70";
        String address = "202.129.238.0";
        System.out.println("IP地址["+address + "]获取到的区域信息:" + ip.getIPLocation(address).getCountry()
